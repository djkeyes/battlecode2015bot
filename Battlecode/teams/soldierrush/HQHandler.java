...
public class HQHandler extends BaseBuildingHandler {
	...
	private void determineAttackSignal() throws GameActionException {
		...
		if (isSet) {
			boolean shouldRetreat = (tankCount <= TANKS_NEEDED_TO_RETREAT) && (soldierCount <= SOLDIERS_NEEDED_TO_RETREAT)
			...
			boolean shouldAttack = ((tankCount >= TANKS_NEEDED_TO_ATTACK) && (soldierCount >= SOLDIERS_NEEDED_TO_ATTACK) && launcherCount >= (LAUNCHERS_NEEDED_TO_ATTACK))
					|| isTimeToPullTheBoys;
			if (shouldAttack) {
